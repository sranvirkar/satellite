...
class InputDateSingleDay extends Component {
  constructor(props) {
    super(props);
    this.state = {
      ...
      isFocused: false,
      inputElementBoundingClientRect: undefined
    };
  }
  ...
  handleChange = (event) => {
    ...
    if (answer.value.length > event.target.value.length) {
      ...
      // console.log('free entry mode on, you backspaced');
    }
  ...
  handleBlur = (event) => {
    const to = event.relatedTarget && event.relatedTarget.closest(".shared-InputDateSingleDay");
  ...
  componentDidMount() {
    this.setState({inputElementBoundingClientRect: ReactDOM.findDOMNode(this).getBoundingClientRect()});
  }

  render() {
    const { field, answer, repeatableIndex, updateAnswer } = this.props;
    const { freeEntryModeEnabled, isDisplayingCalendar, isFocused, inputElementBoundingClientRect } = this.state;
    const validPartialAnswerFormat = /^(0[1-9]|[12][0-9]|3[01]|$|0$|1$|2$|3$)(?:\/|$)(0[1-9]|1[012]|$|0$|1$)(?:\/|$)(\d{0,4}|$)$/g;
    ...
    return (
      <div
        className={`shared-InputDateSingleDay ${isDisplayingCalendar || !answer.validationErrors && !isValidCompleteAnswer && isFocused ? "bring-to-front" : ""}`}
        ...
        { isDisplayingCalendar &&
          <Calendar
            ...
            inputElementBoundingClientRect={inputElementBoundingClientRect} /> }
...
export default InputDateSingleDay;
